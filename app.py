...
def process(df):
    # Filter where Sales Qty CMLY (index 9) is non-null
    ...
    result = df_filtered.groupby(df_filtered.iloc[:, 4]).agg(
        ...
        total_to= (df_filtered.columns[12], 'sum'),      # ← FIXED: col M = Sales Value CMLY
...
# Average SKU Count across 3 months
merged['Average SKU Count'] = (
    merged['sku_jan'] + merged['sku_feb'] + merged['sku_mar']
) / 3

# Average TO = sum of col M (CMLY) across all 3 months / 3
merged['Average TO'] = (
